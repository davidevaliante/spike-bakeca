...
const BonusStripe: FunctionComponent<Props> = ({ bonus, eng = false, countryCode, fromInstagram }) => {
    ...
    const visit = () => {
        ...
        if (fromInstagram) Router.push(`/visita/${bonus.compareCode}/${countryCode}?instagram=true`)
    ...
    const extractLink = () => {
        ...
        const matchingLink = links.find(
            (it) => it.label === `${bonus.compareCode} ${configuration.streamerName} ${countryCode}`
        )
    ...
    return (
        <Container>
            ...
            <Row style={{ marginTop: '.5rem', justifyContent: 'space-around' }}>
                ...
                <WebSiteButton onClick={() => visit()}>{'Visita il sito'}</WebSiteButton>

                <GuideButton onClick={() => visitGuide()}>{'Leggi la Guida'}</GuideButton>
            </Row>
        </Container>
    )
}
